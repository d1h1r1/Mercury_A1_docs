...
#### 15. Set up terminal 485 communication
...
**15.10**  `    tool_serial_set_timeout(max_time)`

- **function:**  Set 485 timeout in milliseconds, default 30ms
- **Parameters**:   max_time (int): timeout
- **Return value:**   NULL

#### 16. servo state value

**16.1** `get_servo_speeds()`

+ **function**：Get joint speed (Only for mycobot 320)
+ **Return value**：  unit step/s

**16.2** `get_servo_currents()`

+ **function**：Get joint current (Only for mycobot 320)
+ **Return value**：   0 ~ 3250 mA

**16.3** `get_servo_status()`

+ **function**：Get joint status (Only for mycobot 320)
+ **Return value**： ` [voltage, sensor, temperature, current, angle, overload]`, a value of 0 means no error, a value of 1 indicates an error
