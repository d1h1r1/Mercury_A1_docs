...
- **Function introduction**
  - **myStudio**：Support users to download/burn/update robot embedded firmware; embedded block visual programming application, convenient for users to control robots through visual programming; support fast motion control of single joint or single coordinate of robot; support fast control of robot IO input and output; support Real-time monitoring of robot status and other functions.

  - **myPanel**：myPanel is an application that controls the Mercury robotic arm by touching the cm4 of the robotic arm base. Its functions include coordinate control, angle control, drag teaching, etc.) to facilitate the operator to interact with the elephant robot
...
- **Connect VNC**
  ...
  - There are two wireless connection modes. The first mode requires an external monitor to do some operations on the system. The specific steps are as follows:： Click **“Disconnect”**，disconnect the default hotspot connection
    ...
    ![](../../resources/5-BasicApplication/5.1/5.1.3.png)

    Click on the WiFi you need to connect to, enter the password

    ![](../../resources/5-BasicApplication/5.1/5.1.4.png)

    After connect successfully，click **"Connection Information"** to check IP address of the robot

    ![](../../resources/5-BasicApplication/5.1/5.1.5.png)

    As shown in the example，**“192.168.10.64”** is the current IP address of the robot

    ![](../../resources/5-BasicApplication/5.1/5.1.3-5.png)

    Connect your PC and the robot to the same WiFi，open the VNC viewer，enter the IP address（examples : input **192.168.10.64**），enter the password **Elephant**，The user name is not specified by default.
...
[← Previous Chapter](../../4-FirstInstallAndUse/4.1_320_PI_firstUse.md) | [Next Section →](../5.2-ApplicationUse/README.md)